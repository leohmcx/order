...
do
  echo " creating!"
	...
	# Create keystores
	keytool -genkey -noprompt -alias $i -dname "CN=$i, OU=test, O=datahub, L=paris, C=fr" -keystore secrets/$i.keystore.jks -keyalg RSA -storepass datahub -keypass datahub  >/dev/null 2>&1
	echo " keytool!"
